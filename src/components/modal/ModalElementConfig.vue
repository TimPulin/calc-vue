<template>
  <div
    class="modal"
    id="modal-element-config"
    tabindex="-1"
    aria-labelledby="modalLabel"
    aria-hidden="true"
  >
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header">
          <h2 class="modal-title fs-5">Прыжок</h2>
          <button
            type="button"
            class="btn-close"
            data-bs-dismiss="modal"
            aria-label="Close"
          ></button>
        </div>
        <div class="modal-body" id="benchmark-options-element">
          <div class="modal__display">
            <div class="modal__element-name">3T+2a+2Lo></div>
            <div class="modal__scores">12.03</div>
          </div>

          <div class="options options--element" id="jump-name">
            <label class="checkbox options__checkbox">
              A
              <input
                value="a"
                type="radio"
                name="jump"
                class="checkbox__input"
              />
            </label>

            <label class="checkbox options__checkbox">
              T
              <input
                value="t"
                type="radio"
                name="jump"
                class="checkbox__input"
              />
            </label>

            <label class="checkbox options__checkbox">
              s
              <input
                value="s"
                type="radio"
                name="jump"
                class="checkbox__input"
              />
            </label>

            <label class="checkbox options__checkbox">
              lo
              <input
                value="lo"
                type="radio"
                name="jump"
                class="checkbox__input"
              />
            </label>

            <label class="checkbox options__checkbox">
              F
              <input
                value="f"
                type="radio"
                name="jump"
                class="checkbox__input"
              />
            </label>

            <label class="checkbox options__checkbox checkbox--checked">
              Lz
              <input
                value="lz"
                type="radio"
                name="jump"
                class="checkbox__input"
              />
            </label>
          </div>

          <div class="options options--element" id="numbers">
            <label class="checkbox options__checkbox">
              1
              <input
                value="1"
                type="radio"
                name="number"
                class="checkbox__input"
              />
            </label>

            <label class="checkbox options__checkbox">
              2
              <input
                value="2"
                type="radio"
                name="number"
                class="checkbox__input"
              />
            </label>

            <label class="checkbox options__checkbox">
              3
              <input
                value="3"
                type="radio"
                name="number"
                class="checkbox__input"
              />
            </label>

            <label class="checkbox options__checkbox">
              4
              <input
                value="4"
                type="radio"
                name="number"
                class="checkbox__input"
              />
            </label>
          </div>

          <div
            class="options options--element space-evenly"
            id="jump-underotation"
          >
            <label class="checkbox options__checkbox">
              &lt;&lt;
              <input
                value="<<"
                type="radio"
                name="number"
                class="checkbox__input"
              />
            </label>

            <label class="checkbox options__checkbox">
              &lt;
              <input
                value="<"
                type="radio"
                name="number"
                class="checkbox__input"
              />
            </label>

            <label class="checkbox options__checkbox lowercase">
              q
              <input
                value="q"
                type="radio"
                name="number"
                class="checkbox__input"
              />
            </label>

            <label class="checkbox options__checkbox lowercase">
              <svg
                class="svg"
                width="16"
                height="16"
                viewBox="0 0 16 16"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path
                  d="M8.8638 0.0460592C7.90812 -0.192861 7.02087 0.530102 6.95631 1.46585C6.88374 2.51744 6.72691 3.48161 6.52757 4.05676C6.4031 4.4159 6.04895 5.06871 5.48879 5.69469C4.93091 6.31812 4.20585 6.87339 3.35693 7.10496C2.68549 7.28812 2 7.86992 2 8.72015V12.7212C2 13.5656 2.68233 14.185 3.44792 14.2662C4.51741 14.3796 5.0118 14.6814 5.51594 14.9891L5.56354 15.0182C5.83632 15.1844 6.14176 15.3675 6.53434 15.5026C6.93094 15.6392 7.39536 15.7205 8 15.7205H11.5C12.4371 15.7205 13.099 15.2426 13.4341 14.6561C13.5982 14.369 13.6875 14.0499 13.6875 13.7439C13.6875 13.5916 13.6647 13.4318 13.6107 13.2795C13.8121 13.0172 13.9915 12.7021 14.0993 12.3786C14.2091 12.0494 14.2709 11.6171 14.1026 11.2304C14.1718 11.0997 14.2235 10.9614 14.262 10.8266C14.3392 10.5564 14.375 10.2594 14.375 9.9705C14.375 9.68161 14.3392 9.38454 14.262 9.11439C14.2275 8.99364 14.1824 8.87012 14.1237 8.75177C14.5183 8.18035 14.5421 7.55086 14.3576 7.01847C14.1524 6.42641 13.6764 5.91891 13.1581 5.74615C12.311 5.46378 11.3547 5.47028 10.6422 5.53505C10.484 5.54943 10.3353 5.56695 10.1994 5.58555C10.6699 3.58771 10.357 1.89202 10.1366 1.07601C9.99923 0.567518 9.58765 0.227022 9.12522 0.111414L8.8638 0.0460592ZM11.5 14.7205H8C7.48931 14.7205 7.13694 14.6525 6.85992 14.5571C6.57887 14.4603 6.35385 14.3287 6.08393 14.1642L6.04402 14.1399C5.48916 13.8014 4.8456 13.4088 3.55337 13.2717C3.22055 13.2365 3 12.9813 3 12.7211V8.72014C3 8.4658 3.2259 8.17723 3.6201 8.0697C4.71546 7.7709 5.59685 7.07352 6.23399 6.36152C6.86886 5.65205 7.29831 4.88662 7.47243 4.38422C7.71504 3.68422 7.8793 2.61627 7.95393 1.53467C7.97893 1.17237 8.31466 0.939535 8.62127 1.01619L8.88269 1.08154C9.04329 1.12169 9.14107 1.22529 9.17119 1.33678C9.39155 2.15255 9.71926 3.98157 9.02566 6.06237C8.96795 6.23549 9.00915 6.42631 9.13314 6.56021C9.25709 6.69406 9.44428 6.74973 9.62127 6.70555L9.62386 6.70492L9.63757 6.70165C9.65031 6.69864 9.67014 6.69406 9.69646 6.68826C9.74911 6.67666 9.82756 6.66022 9.92698 6.64181C10.1262 6.60491 10.4075 6.5605 10.7328 6.53093C11.3953 6.4707 12.189 6.4772 12.8419 6.69482C13.0174 6.75332 13.2914 6.99582 13.4127 7.34594C13.5193 7.65331 13.4986 8.0148 13.1464 8.36693L12.7929 8.72048L13.1464 9.07404C13.189 9.11658 13.2507 9.215 13.3005 9.38909C13.3483 9.55643 13.375 9.75937 13.375 9.97048C13.375 10.1816 13.3483 10.3845 13.3005 10.5519C13.2507 10.726 13.189 10.8244 13.1464 10.8669L12.7929 11.2205L13.1464 11.574C13.1935 11.6211 13.2546 11.7505 13.1507 12.0624C13.053 12.3554 12.8481 12.6653 12.6464 12.8669L12.2929 13.2205L12.6464 13.574C12.6517 13.5793 12.6875 13.6237 12.6875 13.7439C12.6875 13.8599 12.6518 14.0095 12.5659 14.1599C12.401 14.4484 12.0629 14.7205 11.5 14.7205Z"
                />
              </svg>

              <input
                value="null"
                type="radio"
                name="number"
                class="checkbox__input"
              />
            </label>
          </div>

          <div class="options options--element space-evenly" id="jump-edge">
            <label class="checkbox options__checkbox lowercase">
              e
              <input
                value="e"
                type="radio"
                name="number"
                class="checkbox__input"
              />
            </label>

            <label class="checkbox options__checkbox">
              !
              <input
                value="!"
                type="radio"
                name="number"
                class="checkbox__input"
              />
            </label>

            <label class="checkbox options__checkbox lowercase">
              <svg
                class="svg"
                width="16"
                height="16"
                viewBox="0 0 16 16"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path
                  d="M8.8638 0.0460592C7.90812 -0.192861 7.02087 0.530102 6.95631 1.46585C6.88374 2.51744 6.72691 3.48161 6.52757 4.05676C6.4031 4.4159 6.04895 5.06871 5.48879 5.69469C4.93091 6.31812 4.20585 6.87339 3.35693 7.10496C2.68549 7.28812 2 7.86992 2 8.72015V12.7212C2 13.5656 2.68233 14.185 3.44792 14.2662C4.51741 14.3796 5.0118 14.6814 5.51594 14.9891L5.56354 15.0182C5.83632 15.1844 6.14176 15.3675 6.53434 15.5026C6.93094 15.6392 7.39536 15.7205 8 15.7205H11.5C12.4371 15.7205 13.099 15.2426 13.4341 14.6561C13.5982 14.369 13.6875 14.0499 13.6875 13.7439C13.6875 13.5916 13.6647 13.4318 13.6107 13.2795C13.8121 13.0172 13.9915 12.7021 14.0993 12.3786C14.2091 12.0494 14.2709 11.6171 14.1026 11.2304C14.1718 11.0997 14.2235 10.9614 14.262 10.8266C14.3392 10.5564 14.375 10.2594 14.375 9.9705C14.375 9.68161 14.3392 9.38454 14.262 9.11439C14.2275 8.99364 14.1824 8.87012 14.1237 8.75177C14.5183 8.18035 14.5421 7.55086 14.3576 7.01847C14.1524 6.42641 13.6764 5.91891 13.1581 5.74615C12.311 5.46378 11.3547 5.47028 10.6422 5.53505C10.484 5.54943 10.3353 5.56695 10.1994 5.58555C10.6699 3.58771 10.357 1.89202 10.1366 1.07601C9.99923 0.567518 9.58765 0.227022 9.12522 0.111414L8.8638 0.0460592ZM11.5 14.7205H8C7.48931 14.7205 7.13694 14.6525 6.85992 14.5571C6.57887 14.4603 6.35385 14.3287 6.08393 14.1642L6.04402 14.1399C5.48916 13.8014 4.8456 13.4088 3.55337 13.2717C3.22055 13.2365 3 12.9813 3 12.7211V8.72014C3 8.4658 3.2259 8.17723 3.6201 8.0697C4.71546 7.7709 5.59685 7.07352 6.23399 6.36152C6.86886 5.65205 7.29831 4.88662 7.47243 4.38422C7.71504 3.68422 7.8793 2.61627 7.95393 1.53467C7.97893 1.17237 8.31466 0.939535 8.62127 1.01619L8.88269 1.08154C9.04329 1.12169 9.14107 1.22529 9.17119 1.33678C9.39155 2.15255 9.71926 3.98157 9.02566 6.06237C8.96795 6.23549 9.00915 6.42631 9.13314 6.56021C9.25709 6.69406 9.44428 6.74973 9.62127 6.70555L9.62386 6.70492L9.63757 6.70165C9.65031 6.69864 9.67014 6.69406 9.69646 6.68826C9.74911 6.67666 9.82756 6.66022 9.92698 6.64181C10.1262 6.60491 10.4075 6.5605 10.7328 6.53093C11.3953 6.4707 12.189 6.4772 12.8419 6.69482C13.0174 6.75332 13.2914 6.99582 13.4127 7.34594C13.5193 7.65331 13.4986 8.0148 13.1464 8.36693L12.7929 8.72048L13.1464 9.07404C13.189 9.11658 13.2507 9.215 13.3005 9.38909C13.3483 9.55643 13.375 9.75937 13.375 9.97048C13.375 10.1816 13.3483 10.3845 13.3005 10.5519C13.2507 10.726 13.189 10.8244 13.1464 10.8669L12.7929 11.2205L13.1464 11.574C13.1935 11.6211 13.2546 11.7505 13.1507 12.0624C13.053 12.3554 12.8481 12.6653 12.6464 12.8669L12.2929 13.2205L12.6464 13.574C12.6517 13.5793 12.6875 13.6237 12.6875 13.7439C12.6875 13.8599 12.6518 14.0095 12.5659 14.1599C12.401 14.4484 12.0629 14.7205 11.5 14.7205Z"
                />
              </svg>

              <input
                value="null"
                type="radio"
                name="number"
                class="checkbox__input"
              />
            </label>
          </div>

          <div class="options options--element" id="spin-name">
            <label class="checkbox options__checkbox">
              USp
              <input
                value="s"
                type="radio"
                name="spin"
                class="checkbox__input"
              />
            </label>

            <label class="checkbox options__checkbox">
              SSp
              <input
                value="a"
                type="radio"
                name="spin"
                class="checkbox__input"
              />
            </label>

            <label class="checkbox options__checkbox">
              CSp
              <input
                value="t"
                type="radio"
                name="spin"
                class="checkbox__input"
              />
            </label>

            <label class="checkbox options__checkbox">
              LSp
              <input
                value="lo"
                type="radio"
                name="spin"
                class="checkbox__input"
              />
            </label>
          </div>

          <div class="options options--element space-evenly" id="step-name">
            <label class="checkbox options__checkbox">
              StSq
              <input
                value="s"
                type="radio"
                name="spin"
                class="checkbox__input"
              />
            </label>

            <label class="checkbox options__checkbox">
              ChSq
              <input
                value="a"
                type="radio"
                name="spin"
                class="checkbox__input"
              />
            </label>
          </div>

          <div class="modal__wrap-table">
            <!-- <TableJump v-if="elementType === 'jump'" /> -->
            <TableJump />

            <table class="table table--element" v-if="elementType === 'spin'">
              <thead class="thead">
                <tr>
                  <th class="tr__section" colspan="2"></th>

                  <th class="tr__section">
                    <span class="tr__text">Элемент</span>
                  </th>
                  <th class="tr__section">
                    <span class="tr__text">уровень</span>
                  </th>
                  <th class="tr__section" colspan="2"></th>
                </tr>
              </thead>

              <tbody class="tbody">
                <tr class="tr">
                  <td class="tr__section">
                    <div class="tr__wrapper">
                      <label class="checkbox checkbox--option">
                        F
                        <input
                          value="a"
                          type="checkbox"
                          name="fly"
                          class="checkbox__input"
                        />
                      </label>
                    </div>
                  </td>

                  <td class="tr__section">
                    <div class="tr__wrapper">
                      <label class="checkbox checkbox--option">
                        C
                        <input
                          value="a"
                          type="checkbox"
                          name="change"
                          class="checkbox__input"
                        />
                      </label>
                    </div>
                  </td>

                  <td class="tr__section">
                    <div class="tr__wrapper">
                      <button
                        class="button button--editing options-element-caller"
                        data-options-id="spin-name"
                      >
                        CSp
                      </button>
                    </div>
                  </td>

                  <td class="tr__section">
                    <div class="tr__wrapper">
                      <button
                        class="button button--freeze"
                        data-options-id="numbers"
                      >
                        4
                      </button>
                    </div>
                  </td>

                  <td class="tr__section">
                    <div class="tr__wrapper">
                      <label class="checkbox checkbox--option">
                        V
                        <input
                          value="a"
                          type="checkbox"
                          name="v-error"
                          class="checkbox__input"
                        />
                      </label>
                    </div>
                  </td>

                  <td class="tr__section">1.99</td>
                </tr>
              </tbody>
            </table>

            <table class="table table--element" v-if="elementType === 'step'">
              <thead class="thead">
                <tr>
                  <th class="tr__section">
                    <span class="tr__text">Элемент</span>
                  </th>
                  <th class="tr__section">
                    <span class="tr__text">уровень</span>
                  </th>
                </tr>
              </thead>

              <tbody class="tbody">
                <tr class="tr">
                  <td class="tr__section">
                    <button
                      class="button button--editing options-element-caller"
                      data-options-id="spin-name"
                    >
                      StSq
                    </button>
                  </td>

                  <td class="tr__section">
                    <button
                      class="button button--freeze"
                      data-options-id="numbers"
                    >
                      4
                    </button>
                  </td>

                  <td class="tr__section">1.99</td>
                </tr>
              </tbody>
            </table>
          </div>
          <!-- wrap-table -->
        </div>
        <div class="modal-footer">
          <button
            type="button"
            class="button button--reject"
            data-bs-dismiss="modal"
          >
            Закрыть
          </button>

          <button type="button" class="button button-fill">Сохранить</button>
        </div>
      </div>
    </div>
  </div>
</template>

<script>
import TableJump from '@/components/table/table-jump/TableJump.vue';
import { mapState, mapGetters } from 'vuex';

export default {
  components: {
    TableJump,
  },
  computed: {
    ...mapState({
      element: 'editingElement',
      index: 'editingElementIndex',
    }),

    ...mapGetters({
      elementType: 'editingElementType',
    }),
  },
};
</script>
